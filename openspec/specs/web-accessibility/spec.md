...
Define and guard the accessibility of the web app's public auth pages. The
pages must stay usable on assistive technology, enforced by two test tiers: an
always-on, browser-free static baseline (page language, title, single top-level
heading, viewport, labelled form controls) and a gated in-browser axe-core WCAG
2.0/2.1 A/AA audit.
