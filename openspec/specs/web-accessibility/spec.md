...
Define and guard the accessibility of the web app's rendered views. Every page a
user can reach - public auth pages, authenticated account and MFA pages, the
sudo-gated enrolment pages, and the admin pages - must stay usable on assistive
technology, enforced by an automated in-browser audit against every accessibility
standard the audit engine supports, in the E2E test tier.
