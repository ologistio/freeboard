...
Every text-bearing token pairing used by the component layer SHALL meet WCAG AA
contrast (at least 4.5:1 for normal text) in both the light and the dark theme, and
every non-text seal or fill SHALL meet at least 3:1 against its ground. Where a raw
palette value does not clear AA as text, the design system SHALL use an adjusted
token rather than the raw value: status word text SHALL use the semantic on-soft
`-ink` token (not the semantic base, which remains the non-text seal fill), and any
neutral text weight that does not clear AA SHALL be corrected. Corrections SHALL be
mirrored into `stories/Colors.stories.js` so the reference matches the app. This
implements `web-ux-conventions` A1 and the AA-in-both-themes part of A6.

#### Scenario: Load-bearing text pairs clear AA in both themes

- **WHEN** the contrast ratio is computed from the token values for the load-bearing
  text pairs (each text weight on each ground, and each status word on its soft
  ground and on `panel`) in light and in dark
