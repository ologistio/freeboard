...
VendorScope, IntegrationConnection, EvidenceCollector, and AttestationTemplate. A
...
IntegrationConnection naming an unknown vendor) is owned by the `Freeboard.Core`
...
- **WHEN** the user runs `gitops validate` on a directory whose EvidenceCollector
  of `type: integration` names a `connection` id that no IntegrationConnection
  document defines
- **THEN** the command prints a diagnostic naming the EvidenceCollector and the
...
The `gitops sync` command SHALL persist Vendor, VendorScope, IntegrationConnection,
...
IntegrationConnection it referenced, and an absent IntegrationConnection before an
...
- **WHEN** the user runs `gitops sync` on a valid config containing vendors,
  vendor-scopes, integration-connections, and evidence-collectors (some of
...
- **THEN** each resource is persisted and readable with its fields and references
  intact, including an integration connection's provider, base URL, and cadence and
...
- **WHEN** a first `gitops sync` persists the new kinds and a second `gitops sync`
  ...
  and one integration-connection whose only referencing collector was the omitted one
  (keeping its vendor)
...
DeleteAbsent path used for evidence-collectors, integration-connections, and
attestation-templates. Dropping the integration-collector before its
integration-connection exercises the foreign-key-safe delete ordering: the collector
