...
// Runtime Evidence ingest + per-collector machine credentials. Registered alongside the reader; the
// stores share the connection factory (TryAdd) and are resolved lazily per request.
builder.Services.AddEvidenceIngest(freeboardConnectionString);
