#!/usr/bin/env bash
#
# plan-implement-review-loop stop hook
#
# Phase-aware Stop hook for the plan-implement-review loop. Reads session state from
# tmp/plan-implement-review-loop-session.md and decides whether to block (continue
...
# Try tmp/plan-implement-review-loop-session.md relative to git root, then cwd
find_session() {
  local git_root
  git_root="$(git rev-parse --show-toplevel 2>/dev/null || true)"

  if [[ -n "$git_root" && -f "$git_root/tmp/plan-implement-review-loop-session.md" ]]; then
    echo "$git_root/tmp/plan-implement-review-loop-session.md"
    return 0
  fi

  if [[ -f "tmp/plan-implement-review-loop-session.md" ]]; then
    echo "tmp/plan-implement-review-loop-session.md"
    return 0
  fi
...
if [[ -z "$CONTINUATION" ]]; then
  CONTINUATION="You are running the plan-implement-review loop. Read tmp/plan-implement-review-loop-session.md for state and follow the plan-implement-review-loop skill guide. Take the next action for your current mode and round. Mode: ${MODE}. Round: ${ROUND}. Open findings: ${OPEN_FINDINGS}. Open disputes: ${OPEN_DISPUTES}."
fi
...
REASON=$(json_escape "[plan-implement-review-loop] ${PROGRESS} ${CONTINUATION}")
