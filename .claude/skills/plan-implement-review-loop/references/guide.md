# Plan-Implement-Review Loop Guide
...
| **5** | inf | Salience-triggered escalations disabled. Only mandatory escalations apply. |
...
active -> complete
  ...
  Action: (1) final synthesis,
          ...
          (3) archive session to tmp/plan-implement-review-loop-history/session-{id}.md,
...
Output goes to `tmp/plan-implement-review-codex-response.txt`.
...
codex exec "[independent ideation prompt - task only, NO planner draft]" \
  ...
  -C "[project dir]" --json \
  -o tmp/plan-implement-review-codex-response.txt
...
| Session file corrupted | Check `tmp/plan-implement-review-loop-history/` -> reconstruct from continuation block -> inform user, offer restart |
...
The hook reads `tmp/plan-implement-review-loop-session.md` and:
...
Run against `tmp/plan-implement-review-loop-session.md` after session completes:

```bash
scripts/eval.sh [session-path]
# Default: tmp/plan-implement-review-loop-session.md
