...
Copy this to `tmp/plan-implement-review-loop-session.md` at initialization. Replace bracketed values.
...
- Line 1 is machine-parseable state: `plan-implement-review-loop | mode=X | round=N | findings=N | disputes=N | status=X`
